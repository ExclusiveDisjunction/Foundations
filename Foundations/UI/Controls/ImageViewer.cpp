...
namespace Foundations::UI::Controls
{
	...
	ImageViewer::ImageViewer(int XCoord, int YCoord, int Width, int Height, Control* Parent, HINSTANCE ins, HGLOBAL Target) : ImageViewer(XCoord, YCoord, Width, Height, !Parent ? nullptr : *Parent, ins, Target) {}
	...
	LRESULT ImageViewer::Paint()
	{
		...
		float RateX = (img->GetWidth() <= (unsigned)Width ? 1.0f : (float)img->GetWidth() / (float)Width);
		float RateY = (img->GetHeight() <= (unsigned)Height ? 1.0f : (float)img->GetHeight() / (float)Height);

		float ImgWidth = (float)img->GetWidth(), ImgHeight = (float)img->GetHeight();
		if (RateX != 1)
		{
			int Overlap = static_cast<int>((Width / (RateX - (int)RateX + 1.0f)));
			ImgWidth -= Overlap;
		}
		if (RateY != 1)
		{
			int Overlap = static_cast<int>((Height / (RateY - (int)RateY + 1.0f)));
			ImgHeight -= Overlap;
		}
