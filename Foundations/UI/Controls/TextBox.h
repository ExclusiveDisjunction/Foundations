...
namespace Foundations::UI::Controls
{
	...
	public:
		...
		TextBox(int X, int Y, int Width, int Height, Control* Parent, HINSTANCE ins, String Text, StyleSheet Style, TextStyleSheet TextStyle, bool HasScrolls = false, bool IsPassword = false, int MaxCompacity = -1) : TextBox(X, Y, Width, Height, *Parent, ins, Text, Style, TextStyle, HasScrolls, IsPassword, MaxCompacity) {}
		~TextBox();
