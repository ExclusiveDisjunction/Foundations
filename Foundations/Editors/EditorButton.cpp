...
namespace Foundations::Editors
{
	...
	EditorButton::EditorButton(int XCoord, int YCoord, int Width, int Height, HWND Parent, HINSTANCE ins, Editor* Source, EditorHost* Host, AaColor BaseBk)
	{
		if (!_ThisAtom)
			InitBase(ins);
	...
	LRESULT EditorButton::Paint()
	{
		...
		AaColor BkColor = (HasMouse || _Clicked || IsWindowVisible(*Source)) ? Accent3 : _BaseBk;
		...
		HBRUSH BkBrush = CreateSolidBrush(BkColor), Hatchet = CreateHatchBrush(HS_BDIAGONAL, Accent3);
